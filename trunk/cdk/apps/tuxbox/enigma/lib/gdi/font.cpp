...
int eTextPara::appendGlyph(Font *current_font, FT_Face current_face, FT_UInt glyphIndex, int flags, int rflags)
{
	...
	if (nx >= area.right() && rflags & RS_WRAP)
	{
		int cnt = 0;
...
eTextPara::~eTextPara()
{
	clear();
	if (refcnt>=0)
		eFatal("eTextPara: still referenced");
}
...
void eTextPara::blit(gPixmapDC &dc, const ePoint &offset, const gRGB &background, const gRGB &foreground)
{
	...
	else if (target.bpp == 32)
	{
		...
		if (target.clut.data)
		{
			...
			for (int i = 0; i < 16; ++i)
				...
				if (target.clut.data[lookup8[i]].a != 0xFF)
				{
					lookup32[i] = ((target.clut.data[lookup8[i]].a == 0) ? 0xFF :
							((gFBDC::getInstance()->getRampAlpha(254)) ^ 0xFF)) << 24 |
							target.clut.data[lookup8[i]].r << 16 |
							target.clut.data[lookup8[i]].g <<  8 |
							target.clut.data[lookup8[i]].b;
				}
				else
					lookup32[i] = 0;
		}
	...
	clip &= dc.getClip();
	clip &= eRect(area.left() + offset.x(), area.top() + offset.y(), area.width(), area.height()+100);

	//eDebug("eTextPara::blit: area=%d,%d, stride=%d bpp=%d", target.x, target.y, target.stride, target.bpp);
	...
	for (glyphString::iterator i(glyphs.begin()); i != glyphs.end(); ++i)
	{
		...
		if (sx>0)
			...
			{
				if (opcode == 0)		// 4bit lookup to 8bit
				{
					register __u8 *td=d;
					register int ax;
					for (ax=0; ax<sx; ax++)
