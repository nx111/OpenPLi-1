...
public:
	...
	static Signal2< void, ePtrList<eAction>*, eString & > showHelp;
...
protected:
	...
	int result, in_loop, have_focus;

	/*
	 * *UGLY* HACK, we have to use bitfields to ensure binary compatibility for binary plugins
	 *
	 * Just using 16 bit integers wouldn't help, because we're 32bit aligned
	 */
	int just_showing : 16;
	int cornerRound : 16;
...
//#endif
	...
	void clearHelpList();
	void setHelpID(int /*fHelpID*/); /* obsolete */
	static void resizeRoot(const eSize &newsize);
	void searchMultiple(const eString &name, std::vector<eWidget *> &result);
	bool isExecuting() { return in_loop; }
...
class eMultiWidget : public std::vector<eWidget*>
{
public:
	void show()
	{
	...
	void setText(const eString &string)
	{
		for (unsigned int i = 0; i < size(); i++)
		{
